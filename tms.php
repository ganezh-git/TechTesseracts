...
$query = "SELECT truck FROM truckdata WHERE location = 'toll' and  timestamp >= (NOW() - INTERVAL 30 SECOND) ORDER BY timestamp DESC LIMIT 1";
...
$query = "SELECT * FROM truckdata WHERE timestamp >= (NOW() - INTERVAL 30 SECOND)";
$result = $conn->query($query);
...
// Get Dock_Number for trucks in supervisor_cabin
$dock_map = [];
$dock_no=[];
$query = "SELECT Truck_Number, Dock_Number FROM supervisor_cabin";
$result = $conn->query($query);
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $dock_map[$row['Truck_Number']] = $row['Dock_Number'];
        $dock_no[] = $row['Dock_Number'];
    }
}
//print_r($dock_no);
...
<body>
    <table>
                            ...
                            <td>
                                <select id="truck_number" name="truck_number" placeholder="Choose truck number" required>
                                    ...
                                    $query = "SELECT sg.Truck_Number
                                              ...
                                              WHERE sc.Truck_Number IS NULL group by sc.Truck_Number;";
                                    $result = $conn->query($query);
        ...
        <tr>
            <?php for ($i = 1; $i <= 10; $i++): ?>
                <?php
                $dock_class = isset($dock_map[$docks[$i]]) && $dock_map[$docks[$i]] == "D00{$i}" ? 'green-dock' : '';
//                $dock_message = isset($dock_no[0]) && $dock_no[0] == "D00{$i}" ? 'waiting for Truck Number: ' . $docks[$i].$dock_map[$i] : 'Hello';
//                print_r($dock_no);
                ?>
                <td class="dock <?php echo $dock_class; ?>" style="height: 80px; vertical-align: top; position: relative;">
                    Dock <?php echo $i; ?><br>
                    <?php if (!empty($dock_message)): ?>
                        <div class="truck-number">
                            <?php echo $dock_message; ?>
                        </div>
                    <?php elseif (!empty($docks[$i])): ?>
                        <div class="truck-number">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    <table>
                        <tr>
                            ...
                            <select id="truck_number" name="Truck_Number" required>
                                    <option value="TN01 AB1111">TN01 AB1111</option>
                                    <option value="TN01 AB3333">TN01 AB3333</option>
                                    <option value="TN01 AB5555">TN01 AB5555</option>
                                </select>
                        </tr>
                        <tr>
                            ...
                            <td style="padding: 0; text-align: left;">
                                <select id="ble_tag" name="Tag_Number" required>
                                    <option value="BC5729028A5C">BLE Tag 1</option>
                                    <option value="DD340206C455">BLE Tag 3</option>
                                    <option value="F0F8F2CAD52C">BLE Tag 5</option>
                                </select>
                            </td>
                        </tr>
                    </table>
